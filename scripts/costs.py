...
def main():
    ...
    for trigger in range(1, 4):
        ...
        df.to_csv(f'../src/data/{out_file}', index=False)
...
def read_nrel(cost_type, subregions, years):
    ...
    #read in file
    df_raw = pd.read_csv('../dataSources/NREL_Costs.csv', index_col=0)
...
def p2g_system(cost_type, regions, subregions, years):
    ...
    #read in excel file
    source_file = '../dataSources/P2G_FC_Costs.xlsx'
...
def get_usa_variable_cost():
    ...
    df = pd.read_excel('../dataSources/USA_Data.xlsx',
                       sheet_name='VariableCost(r,t,m,y)')
