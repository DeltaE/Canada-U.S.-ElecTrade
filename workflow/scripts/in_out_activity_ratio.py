...
def main():
    ...
    df_iar = pd.read_csv(Path(Path(__file__).resolve().parent,
        '../../resources/InputActivityRatioByTechnology.csv'), index_col=0)
    df_oar = pd.read_csv(Path(Path(__file__).resolve().parent,
        '../../resources/OutputActivityRatioByTechnology.csv'), index_col=0)
    for year in years:
        for subregion in can_subregions:
    ...
    df_iar_out.to_csv(output_dir, index=False)
    ...
    output_dir = Path(Path(__file__).resolve().parent,
        '../../results/data/OutputActivityRatio.csv')
    df_oar_out.to_csv(output_dir, index=False)
